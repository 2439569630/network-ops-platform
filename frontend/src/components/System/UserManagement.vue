        ...
        <div class="main-content">
            ...
            <div class="filter-wrapper">
                <div class="filter-left">
                    <el-input v-model="query.q" placeholder="搜索用户名/昵称/邮箱" :prefix-icon="Search" clearable
                        class="search-input" @keyup.enter="handleSearch" @clear="handleSearch" />
                    <el-select v-model="query.is_approved" placeholder="状态" clearable class="status-select"
                        @change="handleSearch">
            ...
            <div class="pagination-wrapper">
                <el-pagination v-model:current-page="query.page" v-model:page-size="query.page_size" :total="total"
                    ...
                    :small="isMobile" background @current-change="fetchUsers" @size-change="fetchUsers" />
            </div>
        </div>
        ...
        <el-dialog v-model="createVisible" title="新建用户" :width="isMobile ? '90%' : '500px'" class="custom-dialog">
            <el-form :model="createForm" label-position="top" size="large">
                ...
                <el-form-item label="角色">
                    <el-select v-model="createForm.role_ids" multiple placeholder="分配角色" style="width: 100%;">
            ...
            <div v-if="detailData" class="user-detail-view">
                ...
                <div class="detail-list">
                    ...
                    <div class="detail-item">
                        ...
                        <el-tag size="small" :type="detailData.is_approved ? 'success' : 'danger'">{{
                            detailData.is_approved ?
                                '正常' : '封禁' }}</el-tag>
                    </div>
                    <div class="detail-item vertical">
                        <span class="label">角色权限</span>
                        <div class="role-tags">
                            <el-tag v-for="r in (detailData.roles || [])" :key="r.id" size="small" effect="plain">{{
                                r.name
                                }}</el-tag>
                            <span v-if="!detailData.roles || detailData.roles.length === 0"
                                class="empty-text">暂无角色</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <el-form :model="resetPwdForm" label-position="top" size="large" @submit.prevent>
                <el-form-item label="新密码">
                    <el-input v-model="resetPwdForm.password" type="password" show-password placeholder="请输入新密码"
                        :prefix-icon="Lock" @keyup.enter="submitResetPwd" />
                </el-form-item>
            </el-form>
            <template #footer>
...
const toggleStatus = async (row, isSwitch = false) => {
    ...
    try {
        const res = await axios.post('/api/v1/users/status', {
            user_id: row.id,
        ...
        if (isSwitch) row.is_approved = !row.is_approved;
    } finally {
        saving.value = false;
    }
};
...
const handleBatchDelete = async () => {
    ...
    try {
        await ElMessageBox.confirm(`确认删除选中的 ${selectedUserIds.value.length} 个用户？此操作不可恢复。`, '警告', { type: 'warning', confirmButtonClass: 'el-button--danger' });
    } catch { return; }
