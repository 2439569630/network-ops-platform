...
async def upgrade(db: BaseDBAsyncClient) -> str:
    return """
        ...
        ON public.user_roles USING btree (role_id, user_id);
        ...
        BEGIN
            ...
            EXCEPTION WHEN insufficient_privilege THEN
                RAISE NOTICE 'skip pg_trgm extension (insufficient privilege)';
            END;
        END
        $$;
        ...
        BEGIN
            IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_trgm') THEN
                ...
                EXECUTE 'CREATE INDEX IF NOT EXISTS idx_users_nickname_trgm ON public.users USING gin (nickname gin_trgm_ops)';
                EXECUTE 'CREATE INDEX IF NOT EXISTS idx_users_email_trgm ON public.users USING gin (email gin_trgm_ops)';
            END IF;
        END
        $$;
    """
...
async def downgrade(db: BaseDBAsyncClient) -> str:
    return """
        ...
        DROP INDEX IF EXISTS public.idx_users_nickname_trgm;
        DROP INDEX IF EXISTS public.idx_users_username_trgm;
        DROP INDEX IF EXISTS public.idx_user_roles_role_user;
    """
